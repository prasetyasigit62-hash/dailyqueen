...
export default function PointRedemption() {
    ...
    const pointRedemption = loyalty.data[2];
    return (
        <div className="lg:pt-[7.9rem] pt-16">
            <section id="hero-image">
                <div className="flex justify-center">
                    <Image src={pointRedemption.image} alt="hero image" height="0" width="0" sizes="100vw" className="w-full h-auto" priority />
                </div>
            </section>
            <div className="text-white p-5 md:p-8">
                <div className="grid grid-cols-1 md:grid-cols-3">
                    <h1 className="text-3xl md:text-4xl font-bold mb-3 border-b border-white pb-3">{pointRedemption.title}</h1>
                </div>
                {/* eslint-disable-next-line react/no-danger */}
                <div dangerouslySetInnerHTML={{ __html: sanitize(pointRedemption.description) }} />
            </div>
